...
###### Fig. 1) Step to identify reproducible and not reproducible regions considering the boarder of each segment for NCOR1 protein. The genome is scanned using a sliding window apporach. Consecutive segments in between a sum vector of 0 are defined as reproducible is the maximum value is three and not reproducible if the maximum values is lower than 3.
