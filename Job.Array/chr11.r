...
library(data.table)
mydata<-fread("chr11.SMARCA4.txt")
...
options(scipen = 999)
write.table(df.Reproducible,"Reproducible.chr11.SMARCA4.Idr.txt",quote=FALSE,col.names = TRUE,row.names = FALSE,sep="\t")
length(Score)
...
options(scipen = 999)
write.table(df.Not.Reproducible,"Not.Reproducible.chr11.SMARCA4.Idr.txt",quote=FALSE,col.names = TRUE,row.names = FALSE,sep="\t")
